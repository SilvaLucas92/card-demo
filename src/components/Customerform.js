  ...
  return (
    <Formik
      ...
      validationSchema={Yup.object({
        ...
        stateCode: Yup.string().required('Required').max(2, 'Must be 2 characters'),
        countryCode: Yup.string().required('Required').max(3, 'Must be 3 characters'),
        zipCode: Yup.string().required('Required').max(10, 'Must be 10 characters'),
        phoneNumber1: Yup.string().required('Required').max(15, 'Must be 15 characters'),
        phoneNumber2: Yup.string().max(15, 'Must be 15 characters'),
        ssn: Yup.string().required('Required').matches(/^[0-9]{9}$/, 'Must be 9 digits'),
        govtIssuedId: Yup.string().required('Required').max(20, 'Must be 20 characters'),
        dob: Yup.date().required('Required'),
        eftAccountId: Yup.string().required('Required').max(10, 'Must be 10 characters'),
        primaryCardIndicator: Yup.string().required('Required').max(1, 'Must be 1 character'),
        ficoCreditScore: Yup.number().required('Required').min(300, 'Must be at least 300').max(850, 'Must be at most 850'),
      })}
      onSubmit={onSubmit}
    >
      {({ isSubmitting }) => (
        <Form>
          ...
          <div>
            ...
            <ErrorMessage name="addressLine3" component="div" />
          </div>

          <div>
            <label htmlFor="stateCode">State Code</label>
            <Field name="stateCode" type="text" />
            <ErrorMessage name="stateCode" component="div" />
          </div>
          ...
          <div>
            ...
            <ErrorMessage name="govtIssuedId" component="div" />
          </div>
